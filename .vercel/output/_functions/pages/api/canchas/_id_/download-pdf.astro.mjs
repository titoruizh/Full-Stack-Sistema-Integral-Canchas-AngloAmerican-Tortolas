...
const GET = async ({ params, request }) => {
  ...
  try {
    let reemplazarVariables = function(template, datos) {
      return template.replace(/\{\{(\w+)\}\}/g, (match, variable) => {
        const valor = datos[variable];
        return valor !== void 0 && valor !== null ? String(valor) : "";
      });
    }, extraerDatosParaTemplate = function(cancha2, validaciones2 = []) {
      const validacionesBesalco = validaciones2.filter((v) => v.empresa_validadora_id === 2 && v.resultado === "validada").sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime());
      const validacionesLinkapsis = validaciones2.filter((v) => v.empresa_validadora_id === 3 && v.resultado === "validada").sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime());
      const validacionesLlayLlay = validaciones2.filter((v) => v.empresa_validadora_id === 4 && v.resultado === "validada").sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime());
      ...
      console.log("Cancha completa:", JSON.stringify(cancha2, null, 2));
      console.log("Todas las validaciones:", JSON.stringify(validaciones2, null, 2));
      ...
      return {
        ...
        NUMERO_CN: cancha2.numero_informe || "",
        // Información del muro (nombre legible)
        MURO: nombreMuro,
        // Información de sector y nombre
        SECTOR: cancha2.sector || "",
        CANCHA_NAME: (cancha2.nombre_detalle || cancha2.nombre || "").toUpperCase(),
        // Fechas principales
        FECHA_ANGLO: cancha2.created_at ? new Date(cancha2.created_at).toLocaleDateString("es-ES") : "",
        ...
        FECHA_BS: validacionBesalco?.created_at ? new Date(validacionBesalco.created_at).toLocaleDateString("es-ES") : "",
        FECHA2_BS: validacionBesalco2?.created_at ? new Date(validacionBesalco2.created_at).toLocaleDateString("es-ES") : "",
        FIRMA_BS: "[Firma Besalco]",
        FIRMA2_BS: "[Firma Besalco 2]",
        // Linkapsis (Topografía) - usando fechas reales
        NUM_PLK: validacionLinkapsis?.id || "",
        FECHA_LK: validacionLinkapsis?.created_at ? new Date(validacionLinkapsis.created_at).toLocaleDateString("es-ES") : "",
        FECHA2_LK: validacionLinkapsis2?.created_at ? new Date(validacionLinkapsis2.created_at).toLocaleDateString("es-ES") : "",
        ...
        FECHA_LL: validacionLlayLlay?.created_at ? new Date(validacionLlayLlay.created_at).toLocaleDateString("es-ES") : "",
        FECHA2_LL: validacionLlayLlay2?.created_at ? new Date(validacionLlayLlay2.created_at).toLocaleDateString("es-ES") : "",
        ...
        COMENTARIOS_BESALCO: (() => {
          const todasValidacionesBesalco = validaciones2.filter((v) => v.empresa_validadora_id === 2).sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
          return todasValidacionesBesalco[0]?.observaciones || "";
        })(),
        COMENTARIOS_LINKAPSIS: (() => {
          const todasValidacionesLinkapsis = validaciones2.filter((v) => v.empresa_validadora_id === 3).sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
          return todasValidacionesLinkapsis[0]?.observaciones || "";
        })(),
        COMENTARIOS_LLAYLLAY: (() => {
          const todasValidacionesLlayLlay = validaciones2.filter((v) => v.empresa_validadora_id === 4).sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
        ...
        FIRMA_AAJO: usuarioJefeOps ? (/* @__PURE__ */ new Date()).toLocaleDateString("es-ES") : "[Sin asignar]"
      };
    };
    const { data: cancha, error: errorCancha } = await supabase.from("vista_canchas_completa").select("*").eq("id", id).single();
    ...
    const { data: validaciones, error: errorValidaciones } = await supabase.from("validaciones").select("*").eq("cancha_id", id).order("created_at", { ascending: false });
    ...
    let htmlTemplate;
    try {
      htmlTemplate = nativeFs.readFileSync(templatePath, "utf-8");
    } catch (error) {
    ...
    const { data: usuariosAnglo, error: errorUsuarios } = await supabase.from("vista_usuarios_completa").select("*").eq("empresa_id", 1).eq("activo", true);
    if (errorUsuarios) {
