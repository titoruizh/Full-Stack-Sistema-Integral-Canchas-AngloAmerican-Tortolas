...
const GET = async ({ params }) => {
  try {
    ...
    const { data: usuario, error } = await supabase.from("vista_usuarios_completa").select("*").eq("id", parseInt(id)).single();
    if (error) {
...
const PUT = async ({ params, request }) => {
  try {
    ...
    const { data: usuario, error } = await supabase.from("usuarios").update(updateData).eq("id", parseInt(id)).select().single();
    if (error) {
